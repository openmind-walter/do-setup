...
cp ../00-Env/${CLUSTER_NAME}.static.tfvars symbolic_link.${CLUSTER_NAME}.static.tfvars
terraform init
# terraform workspace select ${CLUSTER_NAME}
# if [ $? -eq 0 ]; then
#   echo "Workspace ${CLUSTER_NAME}"
    terraform apply -var-file=symbolic_link.${CLUSTER_NAME}.tfvars -var-file=symbolic_link.${CLUSTER_NAME}.backend.infra.tf -var-file=symbolic_link.${CLUSTER_NAME}.static.tfvars
# else
