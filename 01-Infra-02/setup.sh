...
cp ../00-Env/${CLUSTER_NAME}.infra-02.tfvars symbolic_link.${CLUSTER_NAME}.infra-02.tfvars
terraform init
# terraform workspace select ${CLUSTER_NAME}
# if [ $? -eq 0 ]; then
#   echo "Workspace ${CLUSTER_NAME}"
    terraform apply -var-file=symbolic_link.${CLUSTER_NAME}.tfvars -var-file=symbolic_link.${CLUSTER_NAME}.infra-02.tfvars 
# else
