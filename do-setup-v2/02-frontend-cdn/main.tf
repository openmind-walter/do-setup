...
locals {
  ...
  spaces_base_url = "https://${var.domain_prefix}${local.domain}.${var.region}.digitaloceanspaces.com"
}
...
const ROUTE_MAP = ${jsonencode({
  ...
  v.domain_name => {
    ...
    base_url    = local.spaces_base_url
  }
})};
...
async function handleRequest(request) {
  ...
  /* ================= SPA FALLBACK ================= */
  const isStaticAsset = /\.(js|css|png|jpg|jpeg|gif|ico|svg|woff|woff2|ttf|eot|otf)$/.test(filePath);

  if (response.status === 404 && !isStaticAsset) {
    const fallback = `${"$"}{cfg.base_url}/${"$"}{cfg.spaces_path}/index.html`;
